...
def capture_normal_image(image_filename):
    ...
    os.system(f"libcamera-jpeg --saturation 0.0 -o {filename}")

def capture_sunny_image(image_filename):
    filename = os.path.join(photos_dir, image_filename)
    os.system(f"libcamera-still --shutter 500 --gain 1 --saturation 0.0 --contrast 1.5 -o {filename}")


def capture_night_image(image_filename):
    filename = os.path.join(photos_dir, image_filename)
    os.system(f"libcamera-still --shutter 6000000 --gain 32 --denoise cdn_off --saturation 0.0 -o {filename}")
            ...
            if now.second == 0:
                ...
                try:
                    ...
                    if lux < 10:
                        ...
                        capture_sunny_image(image_filename)
                    else:
