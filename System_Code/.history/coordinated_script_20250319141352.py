...
images_dir = "/home/cscan/Desktop/Project/CoordinatedScript/photos"
...
def main():
    print("DEBUG: Enters the main function")
    bus = smbus2.SMBus(1)

    while True:
        # Get the current time
        print("DEBUG: Enters the loop")
        now = datetime.now()
        
        # Only take a photo and read from the lux sensor if it's the start of a new minute
        if now.second == 0:
            print(f"DEBUG: Capturing data at {now}")
